(function () {
  ...
  if (!dialog || !panel || !input || !results || !status || !clearButton) return;
  ...
  function render(query) {
    ...
    loadIndex().then(function (index) {
      var matches = index
        .map(function (item) {
          return { item: item, score: scoreDocument(item, terms) };
        })
        .filter(function (entry) { return entry.score >= 0; })
        .sort(function (a, b) {
          return b.score - a.score || String(a.item.title).localeCompare(String(b.item.title), 'pt-BR');
        })
        .slice(0, 30);
  ...
  function preferredReturnTarget(trigger) {
    if (trigger && trigger.closest && trigger.closest('[data-drawer-layer]')) {
      return document.querySelector('.toolbar__search') || document.querySelector('[data-drawer-open]');
    }
  ...
  function openDialog(event) {
    ...
    lastFocused = preferredReturnTarget(trigger);
    dialog.hidden = false;
  ...
  function closeDialog() {
    ...
    var fallback = document.querySelector('.toolbar__search') || document.querySelector('[data-drawer-open]');
    var target = lastFocused;

    if (!target || !document.documentElement.contains(target) || (target.closest && target.closest('[inert]'))) {
      target = fallback;
    }
  ...
  dialog.addEventListener('keydown', function (event) {
    ...
    if (event.key === 'Tab') {
      var focusables = Array.prototype.slice.call(panel.querySelectorAll(focusableSelector)).filter(function (item) {
